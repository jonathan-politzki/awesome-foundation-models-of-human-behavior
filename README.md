...
A curated list of papers, models, benchmarks, datasets, and companies for the emerging field of **foundation models of human behavior**: models trained on records of what people actually do — behavioral event logs, life trajectories, transactions, interactions — whose learned representations of *people* transfer across many downstream tasks. The object of study is the model **of** the person (one pretrained representation serving prediction, simulation, personalization, risk, and research), as distinct from foundation models merely used **for** a task like recommendation. This field currently exists in fragments across several communities — industrial recommender systems, computational social science, health/life event modeling, financial ML, user modeling, and simulation — that rarely cite each other. This list is the shared shelf.

## Terminology map

The phrase "behavior(al) foundation model" collides across fields. **False friends** (share words, model something else):

- **Behavior Foundation Models (RL/robotics)** — pretrained *agent control policies* for zero-shot task transfer (whole-body humanoid control, motor primitives), not models of people. See [Fast Imitation via Behavior Foundation Models](https://openreview.net/forum?id=qnWtw3l0jb) (Meta AI, ICLR 2024) and the [humanoid BFM survey](https://arxiv.org/abs/2506.20487) (IEEE TPAMI 2025).
- **Human-Centric Foundation Models (computer vision)** — perception and generation of the human *body's appearance and motion* (pose, avatars, motion synthesis), not decisions. See [the HCFM survey](https://arxiv.org/abs/2502.08556) and its [companion list](https://github.com/HumanCentricModels/Awesome-Human-Centric-Foundation-Models).
- **Brain/neural foundation models** — predict *neural responses* to stimuli, e.g. Meta's [TRIBE v2](https://ai.meta.com/blog/tribe-v2-brain-predictive-foundation-model/), a foundation model of the human brain's responses — encoding, not behavior.
- **World models** — learned models of *environment dynamics* for planning/control; here "world model" appears only in the interpretability sense (emergent internal models of the data-generating process).

**Near-synonyms to embrace** (same object, different communities): Large Behavioral Models / LBM (Unbox AI), Large User Models (Alibaba), foundation models of human cognition (Centaur), general user models (GUM, HCI), user behavior modeling / sequential recommendation (industrial recsys), digital twins of people, agentic twins, silicon sampling (LLM-simulated survey respondents).

## Contents

- [Position & Vision](#position--vision)
- [Industrial Behavioral Foundation Models & Generative Recommenders](#industrial-behavioral-foundation-models--generative-recommenders)
- [Life Trajectories & Event Sequences](#life-trajectories--event-sequences)
- [Financial & Transaction Behavior](#financial--transaction-behavior)
- [Human Simulation & Cognition](#human-simulation--cognition)
- [User Modeling & Personalization](#user-modeling--personalization)
- [Transferable & ID-Free Recommendation](#transferable--id-free-recommendation)
- [Scaling Laws for Behavior](#scaling-laws-for-behavior)
- [Federated & Cross-Silo](#federated--cross-silo)
- [Objectives, Representations & Theory](#objectives-representations--theory)
- [Evaluation & Benchmarks](#evaluation--benchmarks)
- [Surveys of Adjacent Fields](#surveys-of-adjacent-fields)
- [Privacy, Manipulation & Society](#privacy-manipulation--society)
- [Datasets](#datasets)
- [Companies & Ecosystem](#companies--ecosystem)
...
PRs welcome. Criteria: an entry must concern **modeling people from behavioral data** — models trained on records of human behavior whose person-representations transfer across tasks — or be a load-bearing adjacent result (theory, evaluation, privacy, or a false friend worth disambiguating). One line per entry: title, link, venue/org, year, one factual sentence. No marketing language.
