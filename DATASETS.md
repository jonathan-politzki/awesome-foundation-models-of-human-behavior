...
| **[HUMANUAL](https://github.com/zou-group/humanlm)** | daily-life issues, political blogs, chat | 26K users · 216K responses | six public collections unified | HumanLM |
...
| **[PANDORA](https://arxiv.org/abs/2004.04460)** (Reddit personality and demographics; [dataset page](https://psy.takelab.fer.hr/datasets/all/pandora/)) | Reddit comments paired with self-reported traits | large-scale, sparse profile coverage (e.g. gender ~3.2K of ~10K profiles) | one of the only open pairings of years of free text with self-reported personality and demographic ground truth; item is the **subreddit**, not the comment | personality-from-text studies |
...
| Dataset | Unit of behavior | Scale | Why it matters | Used by |
|---|---|---|---|---|
| **[RelBench](https://relbench.stanford.edu/)** | relational database events | 11 databases · ~66 temporal tasks | churn, LTV, purchase prediction from raw relational data, no feature engineering | Relational Deep Learning, KumoRFM |
...
| **[PAN author-profiling shared tasks](https://pan.webis.de/)** | inferring author traits (age, gender, personality and others) from their texts, a long-running shared-task series | stated answer (author-provided or annotated profiles) | the longest-running public evaluation of trait inference from text; a neighbour of person modeling rather than a behavior-prediction benchmark |
...
| **[SimUSER](https://arxiv.org/abs/2504.12722)** | persona-matching user simulation with a knowledge-graph memory and visual perception component | LLM judge / self-consistency | part of the Woven by Toyota series (with AlignUSER and ContextSim, which add real A/B ground truth) |
...
| **[SubPOP](https://arxiv.org/abs/2502.16761)** (ACL 2025) | predicting subpopulation response distributions to public-opinion survey questions, 3,362 questions and 70K subpopulation-response pairs | stated answer | fine-tuning on it cuts the LLM-human distribution gap by up to 46% over prompting baselines and generalizes to unseen surveys and subpopulations |
...
| **[UserBench](https://arxiv.org/abs/2507.22034)** | simulated preference satisfaction in user-facing agent tasks | LLM judge / synthetic key | satisfying a simulated user's preference does not establish agreement with a real user ([code](https://github.com/SalesforceAIResearch/UserBench)) |
...
- **Reading the Benchmarks tables by their new Ground truth column**: of the 60 listed there, 16 are scored against a real logged action (a click, purchase, choice, or A/B outcome), 12 against an LLM judge's opinion, 9 against a survey or interview answer, 9 against a synthetic or researcher-authored key with no real person behind it, 2 against a human rating, 9 mix these, and 3 are not scored benchmarks. A benchmark's name rarely tells you which one it is; this list otherwise does not either, which is the gap this column is meant to close.
